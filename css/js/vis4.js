var vis = {
      ...
      show: function(no) {
          ...
          // contentDiv.load("/modules/conzoom/html/helpfunction.html", SpatialServer.Utils.bind(function() {
            ...
            // jq('head').append('<script src="https://cdn.plot.ly/plotly-latest.min.js"></script>');
              // jq('#testSida2').resizable();
            jq('#testSida2').draggable();
              // jq.getScript("https://cdn.plot.ly/plotly-latest.min.js", function() {
              ...
              function addAll(respObj,coords){
                var myFeatures=[];
              ...
              jq('.updateButton').click(function(){
                  ...
                  if(jq(this).val()=="1"){
                    ...
                    cbKort.getSession().getDatasource(layers["1"][0]).executeSMQL(
                        ...
                        function(resp) {
                          ...
                          if(cbKort.mapObj.map.getLayersByName(layers["1"][0])[0]){
                              cbKort.mapObj.map.removeLayer(cbKort.mapObj.map.getLayersByName(layers["1"][0])[0]);
                          }
                          addAll(resp);
                ...
                cbKort.getSession().getDatasource("ds_gk_pg_olyckor_p").executeSMQL(
                    ...
                    function(resp) {
                      ...
                      if(cbKort.mapObj.map.getLayersByName("Clusters")[0]){
                          cbKort.mapObj.map.removeLayer(cbKort.mapObj.map.getLayersByName("Clusters")[0]);
                      }
                      addAll(resp);
                      ...
                      // cbKort.getSession().getDatasource("ds_gk_pg_olyckor_p").executeSMQL(
                      //     "select skadegrad from this group by skadegrad",
                      //     function(resp) {
                      //       console.log(resp)
                      //
                      //       });
              ...
              function createQuery(){
                ...
                var timequery;
                console.log(jq('input[name=timeval]:checked').val());
